...
if img_file:
    ...
    for i in range(Number_of_Column_Groups):
        ...
        try:
            num_columns = st.number_input(label="Number of Columns", min_value=1, max_value=100, value=1, step=1, key=x+y)
            
            num_rows = st.number_input(label="Number of Rows", min_value=1, max_value=100, value=1, step=1, key=x*y)

            row_number = st.number_input(label="Row Number", min_value=1, max_value=100, value=1, step=1, key=x/y)
        except:
